...
static double calcEntropy(vector<vector<double>> &arr){
    ...
    for(i=0;i<n;i++){
        for(j=0;j<n;j++){
            ...
            ans+=-(arr[i][j])*(log(arr[i][j]));//confirm from sir base of log
        }
    }
    return ans;
}
...
public:
	...
	*/
	...
	void setTermCriteria(long int maxIter=1000000,double epsilon=1e-6){
		svm->setTermCriteria(TermCriteria(TermCriteria::MAX_ITER, 5000000, 1e-6));
	}
	...
	void train(){
		...
		svm->trainAuto(trainingSetFeaturesMat, ROW_SAMPLE, yLabelTrainingMat);
	...
	*/
	void loadTrainedModel(string fileName="SVMTrainedModel1.xml"){
		/*FILE * fptr;
		fptr = fopen(fileName.c_str(),"r");
		if(fptr==NULL){
	...
	float predictIndividual(string fileName){
		...
		cvtColor(img, img, cv::COLOR_RGB2GRAY);
			...
			float x=-10;
			if(svm->isTrained()){
				x= svm->predict(testSetFeaturesMat);
			}
			else{
				printf("SVM Model Not Trained\n");
			}
